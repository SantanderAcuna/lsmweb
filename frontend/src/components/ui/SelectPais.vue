...
/**
 * Selector de país. Mismo patrón fallback que `SelectMunicipio`.
 * Cuando exista `/paises` en backend, `CatalogosApi.paisesTodos()` retornará
 * el array y el componente cambiará automáticamente a `<select>`.
 */
interface Props {
...
const { data, isLoading } = useQuery({
  ...
  queryFn: () => CatalogosApi.paisesTodos(),
...
<template>
  ...
  >
    ...
    <option v-for="p in (data ?? [])" :key="p.id" :value="String(p.id)">
      {{ p.nombre }}
    </option>
  </select>
  <template v-else>
    ...
    <div :id="`${id}-help`" class="form-text">
      Ingrese el ID del país (ISO 3166). Catálogo aún no disponible.
    </div>
  </template>
</template>
